...
program matrix_test_main
  ...
  call assert_equal((/3, 4/), row( reshape( (/1, 2, 3, 4/), (/2,2/)), 2), "can extract row")
  ! Test 3: extract row where numbers have different widths
  call assert_equal((/10, 20/), row( reshape( (/1, 2, 10, 20/), (/2,2/)), 2), &
  ...
  call assert_equal((/8, 7, 6/), row( reshape( (/ 1, 2, 3, 4, 5, 6, 7, 8, 9, 8, 7, 6/),(/3,4/)), 4), &
  ...
  call assert_equal((/3, 6, 9/), column( reshape( (/1, 2, 3, 4, 5, 6, 7, 8, 9/), (/3, 3/)), 3), &
    & "can extract column")
  ! Test 7: can extract column from non-square matrix with no corresponding row
  call assert_equal((/4, 8, 6/), column( reshape( (/1, 2, 3, 4, 5, 6, 7, 8, 9, 8, 7, 6/), (/4,3/)), 4), &
  ...
  call assert_equal((/1903, 3, 479/), column( reshape((/89, 1903, 3, 18, 3, 1, 9, 4, 800/),(/3,3/)), 2), &
