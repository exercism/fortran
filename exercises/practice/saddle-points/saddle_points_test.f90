...
contains
  ...
  subroutine assert_equal_pnt_arr(e_pnt_arr, i_int_arr, test_description)
    type(point_t), intent(in) :: e_pnt_arr(:), i_int_arr(:)
    ...
    assert_test = size(e_pnt_arr) == size(i_int_arr)

    if (.not. assert_test) then
      call test_fail_msg(test_description)
      call elogger('Arrays are not the same size!')
      expected_msg = get_expected_msg(adjustl(pa_to_s(e_pnt_arr)), adjustl(pa_to_s(i_int_arr)))
    ...
    do i = 1, size(e_pnt_arr)
      assert_test = i_int_arr(i) == e_pnt_arr(i)
...
end module saddle_points_helper


program saddle_points_test_main
  use saddle_points_helper
  implicit none
